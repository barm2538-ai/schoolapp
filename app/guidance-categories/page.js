...
export default function GuidanceCategoriesPage() {
  ...
  const handleAddCategory = async (e) => {
    ...
    await addDoc(collection(db, 'guidanceCategories'), { name: newCategory });
    setNewCategory('');
  };

  const handleDelete = async (id) => {
    if (window.confirm("คุณแน่ใจหรือไม่ว่าต้องการลบหมวดหมู่นี้?")) {
      await deleteDoc(doc(db, 'guidanceCategories', id));
    }
  };
  ...
  return (
    <div className="w-full max-w-2xl">
      <h1 className="text-3xl font-bold mb-8">จัดการหมวดหมู่แนะแนว</h1>
