...
export default function SurveysPage() {
  ...
  const handleEdit = (surveyItem) => {
    setEditingSurvey(surveyItem);
    setIsModalOpen(true);
  };
    ...
    <>
      <div className="w-full max-w-5xl">
        <div className="flex justify-between items-center mb-8">
            <h1 className="text-3xl font-bold">จัดการประเมินความพึงพอใจ</h1>
            <button onClick={() => { setEditingSurvey(null); setIsModalOpen(true); }} className="bg-blue-500 text-white px-4 py-2 rounded">
              + เพิ่มแบบประเมินใหม่
            </button>
        </div>
