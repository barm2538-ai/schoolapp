...
export default function LibraryResourceForm({ isOpen, onClose, resourceToEdit }) {
  const [formData, setFormData] = useState({ title: '', description: '', url: '', imageUrl1: '', imageUrl2: '', imageUrl3: '', category: '' });
  ...
  useEffect(() => {
    if (resourceToEdit) {
      ...
      setFormData({ title: '', description: '', url: '', imageUrl1: '', imageUrl2: '', imageUrl3: '', category: categories.length > 0 ? categories[0] : '' });
    }
