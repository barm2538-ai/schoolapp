...
export default function PartnerForm({ isOpen, onClose, partnerToEdit }) {
  const [formData, setFormData] = useState({ name: '', description: '', contactPhone: '', mapUrl: '', imageUrl: '' });
  ...
  useEffect(() => {
    if (partnerToEdit) {
      ...
      setFormData({ name: '', description: '', contactPhone: '', mapUrl: '', imageUrl: '' });
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (partnerToEdit) {
        await setDoc(doc(db, 'partners', partnerToEdit.id), formData, { merge: true });
      } else {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-2xl">
        <h2 className="text-2xl font-bold mb-6">{partnerToEdit ? 'แก้ไขข้อมูลภาคีเครือข่าย' : 'เพิ่มภาคีเครือข่ายใหม่'}</h2>
        <form onSubmit={handleSubmit}>
          <div className="space-y-4 mb-6">
            <input name="name" value={formData.name} onChange={handleChange} placeholder="ชื่อหน่วยงาน" className="w-full px-3 py-2 border rounded-lg" required />
            <textarea name="description" value={formData.description} onChange={handleChange} placeholder="รายละเอียด" className="w-full px-3 py-2 border rounded-lg" rows="3" />
